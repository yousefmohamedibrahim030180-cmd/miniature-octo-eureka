...
(function(){
  ...
  function buildNav(){
    ...
    nav.innerHTML='<div class="aa-nav-label">CORE NAVIGATION</div>'+sections.map((s,i)=>
      '<button class="aa-nav-btn" data-aa-view="'+s.id+'"><span class="aa-nav-icon">'+s.icon+'</span><span class="aa-nav-copy"><strong>'+s.label+'</strong><span>'+s.hint+'</span></span>'+(s.id==="dms"&&Number(window.dmState?.list?.length||0)?'<b class="aa-nav-badge">'+Math.min(99,window.dmState.list.length)+'</b>':"")+'<span class="aa-nav-chevron">›</span></button>'+
  ...
  function renderContext(tab="overview"){
    ...
    if(tab==="overview"){
      ...
      const activity=Array.isArray(window.pulseState?.activity)?window.pulseState.activity.slice(-8).reverse():[];
  ...
  function updateHomeTelemetry(){
    ...
    const users=Array.isArray(window.pulseState?.users)?window.pulseState.users.filter(x=>String(x.status||"online").toLowerCase()!=="offline"):[];
    const activity=Array.isArray(window.pulseState?.activity)?window.pulseState.activity.slice(-7).reverse():[];
  ...
  function boot(){
    ...
    const observer=new MutationObserver(()=>{
      ...
      if(window.orbitUI?.view==="home")buildHomeBoost();
      window.__orbitAscensionSync?.();
    });
    observer.observe($("#app")||document.body,{childList:true,subtree:true});
    setInterval(()=>{if(window.orbitUI?.view==="home")updateHomeTelemetry();refreshContext()},1800);
