...
(function(){
  ...
  function buildNav(){
    ...
    const sync=()=>{
      const active=(orbitUI?.view||"home");
  ...
  function renderContext(tab="overview"){
    ...
    const me=me||{};const ch=currentChannel||{};const srv=currentServer||{};
    const dms=dmState||{};const other=dms.active?.otherUser||null;
    ...
    if(tab==="overview"){
      const online=Number(pulseState?.users?.length||0);
      const calls=Number(pulseState?.calls?.length||0);
      ...
      const activity=Array.isArray(pulseState?.activity)?pulseState.activity.slice(-8).reverse():[];
  ...
  function buildHomeBoost(){
    const body=$("#page-body");if(!body||$("#aa-home-boost")||String(orbitUI?.view||"")!=="home")return;
    const boost=document.createElement("div");boost.id="aa-home-boost";
    const user=me?.display_name||me?.username||"Operator";
  ...
  function updateHomeTelemetry(){
    ...
    const users=Array.isArray(pulseState?.users)?pulseState.users.filter(x=>String(x.status||"online").toLowerCase()!=="offline"):[];
    ...
    $("#aa-messages-count")&&($("#aa-messages-count").textContent=String(dmState?.list?.length||0));
    $("#aa-communities-count")&&($("#aa-communities-count").textContent=String(servers?.length||0));
    $("#aa-calls-count")&&($("#aa-calls-count").textContent=String(pulseState?.calls?.length||0));
