...
function setAtmosphere(k){if(!atmos[k])return;state.atmosphere=k;save();render();toast("Atmosphere • "+atmos[k].name)}
function build(){
 ...
 $("#nexus-intensity").oninput=e=>{state.intensity=+e.target.value;save();render(false)};
 $("#nexus-glass").oninput=e=>{state.glass=+e.target.value;save();render(false)};
 $("#nexus-reduced").onchange=e=>{state.reducedMotion=e.target.checked;save();render(false)};
 $("[data-nexus-atmos]").forEach(b=>b.onclick=()=>setAtmosphere(b.dataset.nexusAtmos));
 $("[data-nexus-surface]").forEach(b=>b.onclick=()=>{state.surface=b.dataset.nexusSurface;save();render();toast("Surface • "+b.textContent)});
 render();
}
